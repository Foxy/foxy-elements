    ...
    states: {
      idle: {
        ...
        on: {
          ...
          changeFModJsonata: { actions: 'changeFModJsonata', target: '.modified' },
          addFModOption: { actions: 'addFModOption', target: '.modified' },
          removeFModOption: { actions: 'removeFModOption', target: '.modified' },
          changeJwtSecret: { actions: 'changeJwtSecret', target: '.modified' },
          changeSessionLifespan: { actions: 'changeSessionLifespan', target: '.modified' }
        },
      },
    ...
    actions: {
      ...
      removeOrigin: actions.assign({
        resource: ({ resource }, { index }: AnyEventObject) => {
          if (!resource) return;
          const newList = resource.allowedOrigins.filter((_, i) => i !== index);
          return { ...resource, allowedOrigins: newList };
        },
      }),
      ...
      changeSessionLifespan: actions.assign({
        resource: ({ resource }, { value }: AnyEventObject) => {
          if (resource) return { ...resource, sessionLifespanInMinutes: value };
        },
      }),
