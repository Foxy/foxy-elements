...
export function NdmRuleWeekdayPicker({
  t,
  locale,
  values,
  onChange,
  ...
  const days = new Array(7)
    .fill(0)
    .map((_, i) => translateWeekday(i, locale, 'short'));

  const getLabelClass = (day: number) => {
    let base =
      'flex items-center justify-center m-xs h-m w-xl rounded font-medium ';
    base += values.includes(day)
      ? 'text-base bg-primary'
      : 'bg-shade-5 text-primary';
    return base;
  };
  ...
  return UI.Group(
    ...
    `,
    UI.If(values.length > 0, () =>
      UI.Hint(
        t('ndmod.dayHint', {
          days: values.map(v => translateWeekday(v, locale)),
        })
      )
    )
  );
