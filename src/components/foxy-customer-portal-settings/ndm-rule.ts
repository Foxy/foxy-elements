...
import { parseDuration, translateWeekday } from '../../common/utils.js';
...
export function NdmRule({ t, rule, locale, modified, disabled, onChange }: NdmRuleParams) {
      ...
      <details>
        <summary class="relative leading-s">
          <div class="p-m text-m text-header font-medium">
            ...
            ${UI.If(
              rule.jsonataQuery !== '*',
              () => html`<code class="inline-block rounded bg-success-10 text-success px-xs text-xs leading-s">${rule.jsonataQuery}</code>`
            )}
            ...
            ${UI.If(
              ...
              () => html`
                <div class="text-s text-secondary font-normal">
                  ...
                  ${parsedMin ? `${parsedMin.count} ${t(parsedMin.units, { count: parsedMin.count })}` : t('ndmod.any')}
                  &mdash;
                  ${parsedMax ? `${parsedMax.count} ${t(parsedMax.units, { count: parsedMax.count })}` : t('ndmod.any')}
                </div>
              `
            )}
            ...
            ${UI.If(
              ...
              () => html`
                <div class="text-s text-secondary font-normal">
                  ...
                  ${UI.If(rule.allowedDays!.type === 'month', () => rule.allowedDays!.days.join(', '))}
                  ${UI.If(rule.allowedDays!.type === 'day', () => rule.allowedDays!.days.map(day => translateWeekday(day, locale, 'short')).join(', '))}
                </div>
              `
            )}
            ...
            ${UI.If(
              ...
              () => html`
                <div class="text-s text-secondary font-normal">
                  ...
                  ${rule.disallowedDates!.map(date => new Date(date).toLocaleDateString(locale, {
                    year: '2-digit',
                    month: 'short',
                    day: 'numeric'
                  })).join('; ')}
          ...
          UI.Section(
            ...
            UI.Group(
              ...
              UI.If(
                ...
                () => html`<div class="px-m">${UI.Warning(t('ndmod.minWarning'))}</div>`
              ),
            ),
            ...
            UI.Group(
              ...
              UI.List({
                ...
                getText: value => new Date(value).toLocaleDateString(locale, {
                  day: 'numeric',
                  month: 'long',
                  year: 'numeric'
                }),
