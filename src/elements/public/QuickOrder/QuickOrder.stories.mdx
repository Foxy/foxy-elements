...
Child products allow you to bundle a set of products together. You can achieve this by nesting `foxy-item` custom tags, or using the `children` product of the product object within the JS array.
...
Here the same example bundle created with tags.
...
### With a new product list property
...
You can add or remove product tags to the form to maintain the form.

<Preview>
  <Story name="Add a tag" >
    {html`
		<div style="max-width: 800px; border: solid thin silver; padding: 1em; margin: 0 auto">
    <foxy-order-form frequencies='["1d", "1w", ".5m", "1m"]'
                     store="jamstackecommerceexample"
      ...
      <button type="button"
              style="background: #ededed; box-shadow: 1px 1px 2px #ddd; border-radius: 6px; padding: 0.3em 1em"
              onclick="var a=document.createElement('foxy-item'); this.parentElement.appendChild(a); a.name = 'New product'; a.price = 3; a.quantity = 2;">
        Add a new product
      </button>
      <foxy-item
        name="Pup Dog Food"
