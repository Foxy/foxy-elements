...
A product must have a `name`, a `price` and a `currency`. The currency will be automatically provided by the `<foxy-quick-form>`. The `description` attribute is used to display a description of the product within the order form. Foxy Cart handles both basic and advanced features and many of them are accessible by setting products parameters. Here are some of the attributes you can use:
