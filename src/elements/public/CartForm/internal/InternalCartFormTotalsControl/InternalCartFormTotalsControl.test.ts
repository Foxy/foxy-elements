...
import { InternalCartFormTotalsControl } from './index';
...
describe('CartForm', () => {
  describe('InternalCartFormTotalsControl', () => {
    ...
    it('imports and defines itself as foxy-internal-cart-form-totals-control', () => {
      expect(customElements.get('foxy-internal-cart-form-totals-control')).to.equal(
        InternalCartFormTotalsControl
      );
    });

    it('extends InternalControl', () => {
      expect(new InternalCartFormTotalsControl()).to.be.instanceOf(InternalControl);
    });

    it('renders total order price', async () => {
      const router = createRouter();
      const wrapper = await fixture<CartForm>(html`
        <foxy-nucleon
          href="https://demo.api/hapi/transactions/0"
          @fetch=${(evt: FetchEvent) => router.handleEvent(evt)}
        >
          <foxy-internal-cart-form-totals-control infer="summary">
      ...
      const control = wrapper.firstElementChild as InternalCartFormTotalsControl;
