  ...
  mdxSource={dedent`
    <foxy-items-form store="jamstackecommerceexample" currency="usd">
      ...
      <div style="margin: 1em">
        ...
        <label>Rate us</label>
        <input name="rate-us" type="number" min="1" max="5" placeholder="1 - 5"></input>
        <div >
        <label>Where did you hear from us? <input type="radio" name="hear" value="streets">Streets</input> <input type="radio" name="hear" value="internet">Internet</input></label>
        </div>
...
Customers may remove items from the form using the quantity button or the remove checkbox. There may be circumstances, however, where the application itself must remove an item from the form. You may simply remove the item directly:

```javascript
myForm.querySelector('[name="Green Tea of the Month"]').remove();
```

You can also achieve this by providing a new items array or you may choose to remove an item. You will need to use the item id called `pid`, like so:
