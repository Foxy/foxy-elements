    ...
    describe('CategoryRestrictionsPageItem', () => {
      describe('CategoryRestrictionsPageItemContent', () => {
        ...
        it('has no default gift card URL', () => {
          expect(new CategoryRestrictionsPageItemContent()).to.have.property('giftCard', '');
        });
        ...
        it("renders unchecked Checkbox element if gift card item category doesn't exist", async () => {
        ...
        it('renders checked Checkbox element if gift card item category exists', async () => {
          const router = createRouter();
          const createMatchRequest = new Request(
            'https://demo.api/hapi/gift_card_item_categories?gift_card_id=0&item_category_id=0',
            {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                item_category_uri: 'https://demo.api/hapi/item_categories/0',
                gift_card_uri: 'https://demo.api/hapi/gift_cards/0',
              }),
            }
          );
        ...
        it('deletes gift card item category once unchecked', async () => {
          const router = createRouter();
          const createMatchRequest = new Request(
            'https://demo.api/hapi/gift_card_item_categories?gift_card_id=0&item_category_id=0',
            {
        ...
        it('creates gift card item category once checked', async () => {
          const router = createRouter();
          const element = await fixture<CategoryRestrictionsPageItemContent>(html`
            <test-category-restrictions-page-item-content
              item-category="https://demo.api/hapi/item_categories/0"
              gift-card="https://demo.api/hapi/gift_cards/0"
              parent="https://demo.api/hapi/gift_card_item_categories?gift_card_id=0&item_category_id=0"
              @fetch=${(evt: FetchEvent) => router.handleEvent(evt)}
            >
            </test-category-restrictions-page-item-content>
          `);
          ...
          expect(element).to.have.nested.property('form.gift_card_uri', element.giftCard);
