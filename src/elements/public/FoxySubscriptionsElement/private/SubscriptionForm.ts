...
export class SubscriptionForm extends HypermediaResource<Subscription> {
  static get scopedElements(): ScopedElementsMap {
    return {
      ...
      'foxy-items': customElements.get(FoxyItemsElement.defaultNodeName),
      'x-i18n': I18N,
    };
  }
  ...
  render(): TemplateResult {
    ...
    return html`
      <div class="space-y-l text-body font-lumo text-m leading-m">
        ...
        <div class="space-y-m">
          <x-i18n ns=${ns} lang=${lang} key="items" class="text-xl font-medium block"></x-i18n>

          <foxy-items
            lang=${lang}
            first=${this.resource._embedded['fx:last_transaction']._links['fx:items'].href}
          >
          </foxy-items>
        </div>
      </div>
