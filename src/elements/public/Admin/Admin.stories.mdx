...
import { customerPortalSettings } from '../CustomerPortalSettings/mocks';
import { bookmark, store, user } from './mocks';
import './index';
...
export const handleRequest = evt => {
  evt.detail.handle(async url => {
    ...
    if (url.endsWith('/stores/8')) return new Response(JSON.stringify(store));
    if (url.endsWith('/users/2')) return new Response(JSON.stringify(user));
    if (url.endsWith('/stores/8/customer_portal_settings')) {
...
<Meta
  title="views | <foxy-admin>"
  decorators={[withKnobs, withWebComponentsKnobs]}
  parameters={{ component: 'foxy-admin', options: { selectedPanel: 'storybookjs/knobs/panel' } }}
/>
