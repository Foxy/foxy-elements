...
export class CategoryRestrictionsPage extends ConfigurableMixin(
  ...
  static get properties(): PropertyDeclarations {
    return {
      ...super.properties,
      couponItemCategories: { type: String, attribute: 'coupon-item-categories' },
      coupon: { type: String },
    };
  }

  couponItemCategories = '';
      ...
      <div class="relative">
        <div class="relative divide-y divide-contrast-10 ml-m">
          ${new Array(limit).fill(0).map((_, index) => {
            ...
            if (category) {
              try {
                const couponCategoryURL = new URL(this.couponItemCategories);
                const categoryURL = new URL(category._links.self.href);
                const categoryID = categoryURL.pathname.split('/').pop() as string;

                couponCategoryURL.searchParams.set('item_category_id', categoryID);
                couponCategoryURL.searchParams.set('limit', '1');

                href = couponCategoryURL.toString();
              } catch {
                // invalid URL, ignore
              }
            }
