...
You can choose between multiple types of storage for the session info: `memory` for in-memory storage (clears on reload), `session` for Session Storage (clears when browser restarts) and `local` for Local Storage (clears after two weeks or later depending on the browser settings). You can also get more extensive logging by increasing `level` (up to 5).
