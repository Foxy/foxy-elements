...
import { ShippingContainerCard } from './ShippingContainerCard';
...
describe('ShippingContainerCard', () => {
  ...
  it('extends TwoLineCard', () => {
    expect(new ShippingContainerCard()).to.be.instanceOf(TwoLineCard);
  });

  it('registers as foxy-shipping-container-card', () => {
    expect(customElements.get('foxy-shipping-container-card')).to.equal(ShippingContainerCard);
  });

  it('has a default i18next namespace of "shipping-container-card"', () => {
    expect(new ShippingContainerCard()).to.have.property('ns', 'shipping-container-card');
  });

  it('renders name in the title', async () => {
    const handleFetch = (evt: FetchEvent) => router.handleEvent(evt);
    const element = await fixture<ShippingContainerCard>(html`
      <foxy-shipping-container-card @fetch=${handleFetch}></foxy-shipping-container-card>
    `);
