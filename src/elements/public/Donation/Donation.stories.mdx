...
Donation uses the `_top` target by default. This approach avoids issues related to the usage of the custom element within `<iframe>`.
...
You may set a value for `target` to change the default behaviour, which is opening the cart or checkout page top window.
