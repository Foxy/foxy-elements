...
export function SignUpScreen({ siteKey, onSignedIn, onBack }: Props) {
  ...
  if (isDone) {
    return (
      <AuthContainer>
        <AuthColumn>
          <AuthTitle>{intl.formatMessage(messages.signUpHeading)}</AuthTitle>
    ...
    <AuthContainer>
      <AuthColumn>
        ...
        <Form onSubmit={handleSubmit} noValidate $maxWidth="none">
          ...
          {/* hCaptcha renders itself into this node, so it stays a plain
              div -- the widget sets its own size and the form's gap is what
              separates it from the fields. */}
